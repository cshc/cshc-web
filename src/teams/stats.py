...
from matches.models import Match, FixtureType
...
def update_southerners_stats_for_season(season):
    ...
    for match in match_qs:
        if not (match.our_team_id in s_lookup):
            s_lookup[match.our_team_id] = Southerner(
                team=match.our_team, season=match.season)
...
def update_participation_stats_for_season(season):
    """ Updates the stats fields embedded in all ClubTeamSeasonParticipation
        model instances.
    """
    participations = ClubTeamSeasonParticipation.objects.by_season(
        season).select_related('team')
...
def update_participation_stats(participation):
    ...
    """
    LOG.info(
        "Updating Club Team Season Participation stats for {}".format(participation))
    participation.reset()
...
def add_match(participation, match):
    ...
    """
    if match.fixture_type == FixtureType.Friendly:
    ...
    elif match.fixture_type == FixtureType.Cup:
