...
class Southerner(models.Model):
    ...
    drawn = models.PositiveSmallIntegerField(
        "Total number of games drawn", default=0)
    lost = models.PositiveSmallIntegerField(
        "Total number of games lost", default=0)

    goals_for = models.PositiveSmallIntegerField("Goals For", default=0)
    goals_against = models.PositiveSmallIntegerField(
        "Goals Against", default=0)

    result_points = models.PositiveSmallIntegerField(
        "Result points", default=0)
    ...
    avg_points_per_game = models.FloatField(
        "Average points per game", editable=False)
    ...
    def clean(self):
        ...
        if self.played() > 0:
            self.avg_points_per_game = (float)(
                self.total_points()) / (float)(self.played())
        else:
