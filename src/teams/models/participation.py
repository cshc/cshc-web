...
class ClubTeamSeasonParticipationManager(models.Manager):
    ...
    def current(self):
        ...
        return self.get_query_set().filter(season=Season.current())
...
class ClubTeamSeasonParticipation(models.Model):
    ...
    division_result = models.CharField(max_length=20, choices=DivisionResult.Choices,
    ...
    friendly_played = models.PositiveSmallIntegerField(
        "Friendly games played", default=0)
    friendly_won = models.PositiveSmallIntegerField(
        "Friendly games won", default=0)
    friendly_drawn = models.PositiveSmallIntegerField(
        "Friendly games drawn", default=0)
    friendly_lost = models.PositiveSmallIntegerField(
        "Friendly games lost", default=0)
    friendly_goals_for = models.PositiveSmallIntegerField(
        "Friendly goals for", default=0)
    friendly_goals_against = models.PositiveSmallIntegerField(
        "Friendly goals against", default=0)

    cup_played = models.PositiveSmallIntegerField(
        "Cup games played", default=0)
    ...
    cup_goals_for = models.PositiveSmallIntegerField(
        "Cup goals for", default=0)
    cup_goals_against = models.PositiveSmallIntegerField(
        "Cup goals against", default=0)

    league_played = models.PositiveSmallIntegerField(
        "League games played", default=0)
    league_won = models.PositiveSmallIntegerField(
        "League games won", default=0)
    league_drawn = models.PositiveSmallIntegerField(
        "League games drawn", default=0)
    league_lost = models.PositiveSmallIntegerField(
        "League games lost", default=0)
    league_goals_for = models.PositiveSmallIntegerField(
        "League goals for", default=0)
    ...
    def __str__(self):
        return str("{} - {}".format(self.team, self.season))

    def clean(self):
        # Sanity checks
        if self.division_result and not self.division:
            raise ValidationError(
                "Division result cannot be set if no division is selected.")

        if not self.division and self.final_pos is not None:
            raise ValidationError(
                "Final position cannot be set if no division is selected.")

        if self.cup_result and not self.cup:
            raise ValidationError(
                "Cup result cannot be set if no cup is selected.")
        ...
        if self.division is not None and (self.team.gender != self.division.gender):
            raise ValidationError("{} is a {} team but {} is a {} division".format(
                self.team, self.team.get_gender_display(), self.division, self.division.get_gender_display()))

        # Make sure the cup gender matches the team gender!
        if self.cup is not None and (self.team.gender != self.cup.gender):
            raise ValidationError("{} is a {} team but {} is a {} cup".format(
                self.team, self.team.get_gender_display(), self.cup, self.cup.get_gender_display()))
