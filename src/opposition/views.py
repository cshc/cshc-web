...
class ClubDetailView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        club_stats = list(ClubStats.objects.select_related(
            'club', 'team').order_by('team__position').filter(club=club))

        # Get all matches against this club
        matches = Match.objects.select_related('our_team', 'opp_team__club', 'venue', 'division__league', 'cup', 'season').filter(
            opp_team__club=club).order_by('date', 'time')

        # Get all appearances against this club
        appearances = Appearance.objects.select_related(
            'member__user').filter(match__opp_team__club=club)

        # Get all the award winners against this club
        award_winners = MatchAwardWinner.objects.select_related(
            'member__user', 'award').filter(match__opp_team__club=club)
        ...
        [all_team_fixtures.setdefault(ms.match.our_team, []).append(
            ms) for _, ms in match_lookup.iteritems()]
        for team, fixtures in all_team_fixtures.iteritems():
            all_team_fixtures[team] = sorted(
                fixtures, key=lambda f: f.match.date)

        # Shift the totals column to the end of the list
        if club_stats:
            club_totals = next(
                stat for stat in club_stats if stat.is_club_total())
