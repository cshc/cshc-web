...
class Command(BaseCommand):
    ...
    """
    help = "Imports fixtures from a CSV file. View source code for details of the expected data format."
                ...
                while True:
                    ...
                    try:
                        fixture_date = datetime.strptime(
                            date_field, "%d-%b-%y").date()
                    except:
                        if date_field != '':
                            print(
                                "ERROR: Could not parse date field: {}".format(date_field))
                        continue
                    ...
                    try:
                        ...
                        print(
                            "ERROR: Could not find season for date: {}".format(date_field))
                        continue
                    ...
                    for club_team in club_teams:
                        self._parse_fixture(simulate, fixture_season, fixture_date,
                                            club_team, csv_row[team_offset:team_offset+len(OFFSET)])
    ...
    def _cache_teams(self, reader):
        ...
        if len(club_teams) < 1:
            raise ParseException(
                "FATAL ERROR: Could not find team names on row 2")
        ...
        for team in club_teams:
            try:
                ...
                raise ParseException(
                    "FATAL ERROR: Unrecognised team name: {}".format(team['name']))
    ...
    def _parse_fixture(self, simulate, fixture_season, fixture_date, club_team, fields):
        ...
        if fields[OFFSET['fixture_type']] == "":
            print("No match for {} on {}".format(
                club_team['name'], fixture_date))
            return
        ...
        try:
            ...
            print("ERROR: Could not find opposition team with name '{}'".format(
                opp_team_name))
            return
        ...
        if not simulate:
            ...
            match = Match(season=fixture_season, opp_team=opp,
                          our_team=our_team, date=fixture_date)

        # Match home/away designation
        try:
            match.home_away = HOME_AWAY[home_away.lower()]
        except:
            print("ERROR: Invalid Home/Away designation '{}' for {} on {}".format(
                home_away, team_name, fixture_date))
            return
        ...
        try:
            ...
            print("ERROR: Invalid fixture type designation '{}' for {} on {}".format(
                fixture_type, team_name, fixture_date))
            return

        # Match time (can be null)
        if fixture_time_str:
            try:
                match.time = datetime.strptime(
                    fixture_time_str, "%H:%M").time()
            except:
                print("ERROR: Could not parse fixture time '{}' for {} on {}".format(
                    fixture_time_str, team_name, fixture_date))
                return
        else:
            match.time = None
