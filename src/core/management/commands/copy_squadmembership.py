...
class Command(BaseCommand):
    ...
    help = "Copies all Squad Membership records from the previous season to the current season."
    ...
    def _handle(self, *args, **options):
        ...
        # Save has the quirky effect of updating the original object (memberships), so cache the 'before' count.
        membership_count = memberships.count()

        if not options['simulate'] and memberships.exists():
            print(
                "ERROR: SquadMembership entries already exist for {0}. To run this command (non-simulated), first delete all SquadMembership entries for {0}".format(season))
            return -1
        ...
        updated_membership_count = SquadMembership.objects.by_season(
            season).count()
        print("Squad memberships summary: Current season: %d (was %d before script); previous season: %d" % (
            updated_membership_count, membership_count, prev_memberships.count()))
        return 0
