    ...
    def clean(self):
        ...
        if self.our_team and self.opp_team:
            raise ValidationError(
                "You cannot specify both our team and an opposition team ({})".format(self))
        if not self.our_team and not self.opp_team:
            raise ValidationError(
                "You cannot specify neither our team nor an opposition team ({})".format(self))

        if (self.won + self.drawn + self.lost) != self.played:
            raise ValidationError(
                "Won + Drawn + Lost not equal to Played ({})".format(self))

        if (self.goals_for - self.goals_against) != self.goal_difference:
            raise ValidationError(
                "Goals For - Goals Against not equal to Goal Difference ({})".format(self))

    def __str__(self):
        return str("{} - {} ({})".format(self.team_name, self.division, self.season))

    @property
    def team_name(self):
        """ Gets the team name."""
        if self.our_team is None:
            return self.opp_team.genderless_name() if self.opp_team is not None else "Unknown team"
